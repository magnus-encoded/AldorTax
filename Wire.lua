-- Wire — pure codec for AldorTax's sync (S) and death (D) addon-message frames.
-- No WoW API calls, no side effects, no state beyond the version tables. The
-- caller supplies every field already computed; Wire only formats and parses.
--
...
-- Decode an S or D frame into a fully-populated table, or nil when the frame is
-- too short, isn't an S/D frame, or carries a version outside KNOWN_MSG_VERSIONS.
function Wire.Parse(message)
    if not message or #message < 3 then return nil end
    local tag = message:sub(1, 1)
    if tag ~= "S" and tag ~= "D" then return nil end
